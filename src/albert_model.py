...
class AlbertGroup(nn.Cell):
    """AlbertGroup for parameter Sharing"""
    def __init__(self,
                 inner_group_num,
                 hidden_size=768,
                 seq_length=512,
        ...
        self.inner_group_num = inner_group_num
        self.inner_group = nn.CellList([AlbertEncoderCell(
            ...
            compute_type=compute_type) for _ in range(inner_group_num)])
...
class EmbeddingPostprocessor(nn.Cell):
    ...
    def __init__(self,
        ...
        self.token_type_embedding = nn.Embedding(
            ...
            embedding_table=TruncatedNormal(initializer_range))
        self.shape_flat = (-1,)
        self.one_hot = P.OneHot()
        self.on_value = Tensor(1.0, mstype.float32)
...
class AlbertTransformer(nn.Cell):
    ...
    def __init__(self,
        ...
        self.hidden_size = hidden_size
        self.embedding_size = embedding_size
        self.dense_layer_2d = nn.Dense(embedding_size,
                                       hidden_size,
                                       weight_init=TruncatedNormal(initializer_range)).to_float(mstype.float32)
        self.num_hidden_layers = num_hidden_layers
        ...
        self.group = nn.CellList([AlbertGroup(inner_group_num,
                                              ...
                                              use_relative_positions=use_relative_positions,
                                              hidden_act=hidden_act,
                                              compute_type=compute_type) for _ in range(num_hidden_groups)])
        self.get_shape = P.Shape()

    def construct(self, input_tensor, attention_mask):
        """Multi-layer albert transformer."""
        embedding_size = self.get_shape(input_tensor)[2]
        if embedding_size != self.hidden_size:
            # [32,128,768]
            prev_output = self.dense_layer_2d(input_tensor)
        else:
...
class AlbertModel(nn.Cell):
    ...
    def __init__(self,
        ...
        self.albert_embedding_lookup = nn.Embedding(
    ...
    def construct(self, input_ids, token_type_ids, input_mask):
        ...
        # [30000,128]
        embedding_tables = self.albert_embedding_lookup.embedding_table
        # [32,128,128]
        word_embeddings = self.albert_embedding_lookup(input_ids)
        # [32,128,128]
        ...
        encoder_output = self.albert_encoder(
            self.cast_compute_type(embedding_output),
            attention_mask)
        sequence_output = self.cast(encoder_output[self.last_idx], self.dtype)

        batch_size = self.get_shape(input_ids)[0]
