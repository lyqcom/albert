...
from src.albert_for_finetune import AlbertSquadV2
...
args_opt = get_config("../task_squad_v2_config.yaml")
...
def do_train(dataset=None, network=None, load_checkpoint_path="", save_checkpoint_path="", epoch_num=1):
    ...
    ckpoint_cb = ModelCheckpoint(prefix="squadV2",
    ...
    # netwithgrads = AlbertSquadV2Cell(network, optimizer=optimizer)
...
def do_eval(save_finetune_checkpoint_path, squadprocessor, tokenizer):
    ...
    from src.squad_get_predictions import accumulate_predictions_v2
    from src.squad_postprocess import evaluate_v2
    ...
    def save_finetune_checkpoint():
        ...
        for filepath, _, filenames in os.walk(save_finetune_checkpoint_path):
            for index, checkpoint in enumerate(filenames):
                ...
                net = AlbertSquadV2(albert_net_cfg, False, 2)
                net.set_train(False)
                ...
                model = Model(net)
                output = []
                RawResult = collections.namedtuple("RawResultV2",
                                                   ["unique_id", "start_top_log_probs", "start_top_index",
                                                    "end_top_log_probs", "end_top_index", "cls_logits"])
                columns_list = ["input_ids", "input_mask", "segment_ids", "unique_id", "p_mask"]
                for data in ds.create_dict_iterator(num_epochs=1):
                    input_data = []
                    for i in columns_list:
                        input_data.append(data[i])
                    input_ids, input_mask, segment_ids, unique_ids, p_mask = input_data
                    ...
                    logits = model.predict(input_ids, input_mask, segment_ids, start_positions,
                                           end_positions, unique_ids, is_impossible, p_mask)
                    ids = logits[0].asnumpy()
                    start_top_log_probs = logits[1].asnumpy()
                    start_top_index = logits[2].asnumpy()
                    end_top_log_probs = logits[3].asnumpy()
                    end_top_index = logits[4].asnumpy()
                    cls_logits = logits[5].asnumpy()

                    for i in range(args_opt.eval_batch_size):
                        unique_id = int(ids[i])
                        start_probs = ([float(x) for x in start_top_log_probs[i].flat])
                        start_index = [int(x) for x in start_top_index[i].flat]
                        end_probs = ([float(x) for x in end_top_log_probs[i].flat])
                        end_index = [int(x) for x in end_top_index[i].flat]
                        cls = float(cls_logits[i].flat[0])
                        output.append(RawResult(
                            unique_id=unique_id,
                            start_top_log_probs=start_probs,
                            start_top_index=start_index,
                            end_top_log_probs=end_probs,
                            end_top_index=end_index,
                            cls_logits=cls))

                result_dict = {}
                cls_dict = {}
                accumulate_predictions_v2(result_dict, cls_dict, eval_examples, eval_features, output,
                                          args_opt.n_best_size,
                                          args_opt.max_query_length,
                                          args_opt.start_n_top, args_opt.end_n_top)
                ...
                output_null_log_odds_file = os.path.join(
                    args_opt.output_path, "null_odds.json")

                result = evaluate_v2(result_dict, cls_dict, prediction_json, eval_examples,
                                     eval_features, output, args_opt.n_best_size,
                                     args_opt.max_answer_length, output_prediction_file, output_nbest_file,
                                     output_null_log_odds_file)

                if result['f1'] > best_result_f1:
                    best_result_f1 = result['f1']
                    best_result_em = result['exact']
                    if best_checkpoint != "":
                ...
                del ds, net, param_dict, model, result_dict, cls_dict, output
                gc.collect()
    ...
    print("==============================================================")
    print("squad_v2 best eval f1/em :", best_result_f1, best_result_em)
    print("==============================================================")
    save_path = os.path.join(save_finetune_checkpoint_path, "squad_v2_result.txt")
    ...
    with open(save_path, 'a+') as wr:
        if os.path.exists(save_path):
            wr.write("\n")
        wr.write(
            "squad_v2 {} epoch eval result {} {}".format(str(args_opt.epoch_num), best_result_f1, best_result_em))
...
def run_squad():
    ...
    netwithloss = AlbertSquadV2(config=albert_net_cfg,
                                is_training=True,
                                num_labels=2,
                                dropout_prob=0.1,
                                start_n_top=args_opt.start_n_top,
                                end_n_top=args_opt.end_n_top)
